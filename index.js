...
Object.defineProperty(Musician.prototype, "band", {
  set: function (newBand) {
    if (typeof newBand !== "string") {
      throw new TypeError("Властивість band має бути string.");
    }
    this.band = newBand;
  },
});
...
class Band {
  ...
  addMember(newMember) {
    if (
      ...
      newMember instanceof Guitarist ||
      newMember instanceof Bassist
    ) {
      // Перевірка, чи newMember є екземпляром Musician або його нащадків
      this.#members.push(newMember);

      // Використання сеттера band класу Musician для встановлення назви групи для нового учасника
  ...
  playMusic() {
    console.log(`Гурт ${this.name} розпочинає грати!`);
 ...
 * | band        | "Led Zeppelin"    |
 */
class guitarist extends Guitarist {
  name = "Jimmy Page";
  instrument = "гітара";
  band = "Led Zeppelin";
}
 ...
 * | band        | "Queen"           |
 */
class vocalist extends Vocalist {
  name = "Freddie Mercury";
  band = "Queen";
}
...
class songwriter extends SongWriter {
  songs = ["Yesterday", "Hey Jude", "Let It Be"];
}
 ...
 */
class performance extends Performance {
  band = band;
  location = "Liverpool";
  date = new Date("2023-08-01");
}
 ...
 * | songs       | ["Yesterday","Hey Jude","Let It Be",]|
 */
class leadsinger extends LeadSinger {
  name = "Mick Jagger";
  band = "The Rolling Stones";
  songs = ["Yesterday", "Hey Jude", "Let It Be"];
}
